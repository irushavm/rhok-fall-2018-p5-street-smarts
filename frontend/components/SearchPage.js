      ...
      <ContextProvider>
        <AppContext.Consumer>
          {context => {
            if (searchType === 'sponsor') {
              ...
              return (
                <div>
                  <List>
                    {users
                      .filter(e => e.type === searchType)
                      .map(({ name, candidateAgency, id, email }, ind) => (
                        <ListItem key={`${ind}`}>
                              ...
                              <Grid container item xs={8}>
                                ...
                                <Grid container item xs={12}>
                                  <Grid item xs={6} align="center">
                                    <Link
                                      href={`/user/details?type=candidate&id=${id}`}
                                    >
                                  ...
                                  <Grid item xs={6} align="center">
                                    <a href={`mailto:${email}`}>
                                      <TextLink>Get in Touch</TextLink>
                                    </a>
                                  </Grid>
                                </Grid>
                              </Grid>
            ...
            const employerList = context.state.users.filter(
              e => e.type === 'employer'
            );
            return (
              <div>
                <List>
                  ...
                  context.state.jobs
                    .map(jobObject => ({
                      ...jobObject,
                      employerName: employerList.find(
                        employerObject =>
                          employerObject.id === employerObject.id
                      ).name
                    }))
                    .map(
                      (
                        { id, title, description, status, employerName },
                        ind
                      ) => (
                        <ListItem key={`${ind}`}>
